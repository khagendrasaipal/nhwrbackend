...
public class IndicatorService extends AutoService {
	...
	public ResponseEntity<Map<String, Object>> store() {
		...
		sql = "INSERT INTO indicator (cat_id,code,name) VALUES (?,?,?)";
		DbResponse rowEffect = db.execute(sql,
				Arrays.asList(model.cat_id,model.code, model.name));
	...
	public ResponseEntity<Map<String, Object>> edit(String id) {
		...
		String sql = "select id,code,name,cat_id from "
				+ table + " where id=?";
	...
	public ResponseEntity<Map<String, Object>> update(String id) {
		...
		String sql = "UPDATE indicator set code=?,name=?,cat_id=? where id=?";
		rowEffect = db.execute(sql,
				Arrays.asList(model.code, model.name,model.cat_id,id));
