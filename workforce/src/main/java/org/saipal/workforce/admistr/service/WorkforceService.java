...
//				+ " tbl_upasamuha.id=td.subgroupid join tbl_post on tbl_post.id=td.post  left join tbl_employee on tbl_employee.detailsid=td.id left join tbl_emptype on tbl_emptype.id=tbl_employee.emptype left join tbl_level on tbl_level.id=tbl_employee.level left join tbl_council on tbl_council.id=tbl_employee.council where td.workforce_id=?";
		...
		String sql = "select COALESCE(hfregistry.hf_name,'') as hf_name,COALESCE(tbl_level.namenp,'') as level,COALESCE(tbl_emptype.namenp,'') as emptype,COALESCE(tbl_council.namenp,'') as council,COALESCE(tbl_office.namenp,'')as orgname,tbl_samuha.namenp as groupname,COALESCE(tbl_upasamuha.namenp,'') as subgroup,tbl_post.namenp as postname,COALESCE(tbl_employee.namenp,'') as empname,"
				...
				+ " tbl_upasamuha.id=td.subgroupid join tbl_post on tbl_post.id=td.post   join tbl_employee on tbl_employee.detailsid=td.id left join tbl_emptype on tbl_emptype.id=tbl_employee.emptype left join tbl_level on tbl_level.id=tbl_employee.level left join tbl_council on tbl_council.id=tbl_employee.council where td.workforce_id=?";
	...
	public void downloadreport2(HttpServletResponse response) throws IOException {
...
//		}
		...
		cell = row.createCell(1);
		cell.setCellValue("स्वीकृत दरबन्दी");

		cell = row.createCell(2);
		cell.setCellValue("स्वीकृत दरबन्दीमा कार्यरत");

		cell = row.createCell(3);
		cell.setCellValue("दरबन्दी बाहेकको कार्यरत संख्या");
		
		cell = row.createCell(4);
		cell.setCellValue("जम्मा कार्यरत संख्या");
		   ...
		   Map<Integer, Object> map;
			...
			for(String fld:flds) {
				sheet.addMergedRegion(new CellRangeAddress(
						...
						columnCount+2  //last column  (0-based)
				));
				cell = row.createCell(columnCount++);
...
//			cell.setCellValue("Province/PostType");
			for(String fld:flds) {
				...
				cell.setCellValue("स्वीकृत दरबन्दी");
				cell = row.createCell(columnCountnew++);
				cell.setCellValue("दरबन्दीमा कार्यरत");
				cell = row.createCell(columnCountnew++);
				cell.setCellValue("अन्य कार्यरत");
			}
			...
			for (Tuple t : admlvl) {
				...
				for(String fld:flds) {
					...
					cell = row.createCell(columnCount++);
					cell.setCellValue(t.get("w"+fld)+"");
					cell = row.createCell(columnCount++);
					cell.setCellValue(t.get("n"+fld)+"");
...
//			return Messenger.getMessenger().setData(datas).success();
	...
	public void downloadreportpivot(HttpServletResponse response) throws IOException {
...
//		System.out.println(flds);
		...
		Cell cell = row.createCell(0);
		   ...
		   Map<Integer, Object> map;
			...
			row = sheet.createRow(rowCount++);
			cell = row.createCell(columnCountnew++);
			if(request("row").equals("province")) {
				cell.setCellValue("Province/PostType");
			}
			if(request("row").equals("district")) {
				cell.setCellValue("District/PostType");
			}
			if(request("row").equals("palika")) {
				cell.setCellValue("Local Level/PostType");
			}
//			cell.setCellValue("Province/PostType");
			for(String fld:flds) {
			...
			for (Tuple tts : admlvl) {
				...
				columnCount = 0;
				cell = row.createCell(columnCount++);
				cell.setCellValue(tts.get("provincename")+"");
				for(String fld:flds) {
					cell = row.createCell(columnCount++);
					cell.setCellValue(tts.get("d"+fld)+"");
					cell = row.createCell(columnCount++);
					cell.setCellValue(tts.get("w"+fld)+"");
					cell = row.createCell(columnCount++);
					cell.setCellValue(tts.get("n"+fld)+"");
//					cell.setCellValue(Integer.parseInt(t.get("k"+fld).toString())+Integer.parseInt(t.get("d"+fld).toString()));
				}
	...
	public void downloadreportpivot2(HttpServletResponse response) throws IOException {
//		String sql="";
